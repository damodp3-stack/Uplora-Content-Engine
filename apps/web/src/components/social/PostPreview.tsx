...
import { Twitter, Linkedin, Instagram, Heart, MessageSquare, Repeat, Send } from 'lucide-react';
...
export function PostPreview({ content, title }: PostPreviewProps) {
  ...
  const formattedText = content || title || 'Your preview message will render here in real-time...';
        ...
        <div className="flex items-center gap-1 bg-gray-900/60 p-1 rounded-xl border border-gray-800">
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${
              activePlatform === 'twitter' ? 'bg-sky-500/20 text-sky-400 font-bold' : 'text-gray-400'
            }`}
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${
              activePlatform === 'linkedin' ? 'bg-blue-500/20 text-blue-400 font-bold' : 'text-gray-400'
            }`}
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${
              activePlatform === 'instagram' ? 'bg-pink-500/20 text-pink-400 font-bold' : 'text-gray-400'
            }`}
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-white flex items-center gap-1">
              Uplora Engine <span className="text-purple-400 text-[10px]">✔</span>
            </p>
            <p className="text-[10px] text-gray-500">
              {activePlatform === 'twitter' ? '@uplora_ai' : 'Content Marketing AI'}
            </p>
          </div>
        </div>
