  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleAddComment(
    @ConnectedSocket() client: Socket,
    @MessageBody() payload: { contentId: string; text: string; selectedText?: string },
  ) {
    const user = this.connectedUsers.get(client.id);
    if (!user) return;
    ...
    this.server.to(`content:${payload.contentId}`).emit('comment:added', comment);
  }
  ...
  private getPresentUsers(contentId: string) {
    ...
    return Array.from(room)
      .map((socketId) => {
        ...
        return u ? { userId: u.userId, socketId, name: u.name, color: u.color, cursor: u.cursor } : null;
      })
      .filter(Boolean);
  }
}
