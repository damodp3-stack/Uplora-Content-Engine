...
import { SocialPublisherService, PublishPayload } from './social-publisher.service';
...
export class SocialPublisherController {
  constructor(private readonly socialPublisherService: SocialPublisherService) {}

  @Post('publish')
  @ApiOperation({ summary: 'Publish content immediately to selected social platforms' })
  async publish(@Body() payload: PublishPayload) {
    return this.socialPublisherService.publishToPlatforms(payload);
  }

  @Get('accounts')
  @ApiOperation({ summary: 'Get list of connected social media platform accounts' })
